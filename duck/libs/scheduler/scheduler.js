// from https://github.com/artsy/day-schedule-selector
(function ($) {
  'use strict';
  ...
  DayScheduleSelector.prototype.renderRows = function () {
    ...
    $.each(generateDates(start, end, interval), function (i, d) {
      var daysInARow = $.map(new Array(days.length), function (_, i) {
        ...
        var tooltip = stringDays[i]+" "+hhmm(d)+ "-" + hhmm(d2);
        return '<td class="time-slot" title="' + tooltip + '"  data-time="' + hhmm(d) + '" data-day="' + days[i] + '"></td>'
      }).join();

      var label = hhmm(d);
      if (label == "00:00") {
        $el.append('<tr><td class="hidden-time-label time-label">' + label + '</td>' + daysInARow + '</tr>');
      }else{
        $el.append('<tr><td class="time-label">' + label + '</td>' + daysInARow + '</tr>');
  ...
  DayScheduleSelector.prototype.clear = function () {
    $.each($('.time-slot'), function( index, value ) {
      $(value).removeAttr('data-selected');
    });
  };

  DayScheduleSelector.prototype.randomize = function () {
    $.each($('.time-slot'), function( index, value ) {
      $(value).removeAttr('data-selected');
      if(Math.random()<=0.5){
        $(value).attr('data-selected', 'selected');
      }
    });
  };
